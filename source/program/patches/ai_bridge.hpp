...
// SWSH AI Bridge V15 - first species-aware switch policy.
//
...
namespace AIBridge {
    ...
    static inline bool IsKnownFriendlyCandidateSpecies(u16 species, u16 active_species, u16 switch_species) {
        if (species == 0) return true;
        if (species == active_species || species == switch_species) return true;
        // Ian/062 known team.  This prevents ally Gallade from being treated as
        // an opposing threat during the current test fight while still allowing
        // all player-side targets like Cinderace/Grimmsnarl to score as threats.
        if (active_species == 620 || active_species == 475 || active_species == 534 || active_species == 448 || active_species == 675) {
            if (species == 620 || species == 475 || species == 534 || species == 448 || species == 675) return true;
        }
        return false;
    }
    ...
    static inline s32 ScoreSwitchMatchupV16(u16 active_species, u16 switch_species, const char* tag, u32 hit) {
        ...
        if (!HasSpeciesTypes(switch_types) || !HasSpeciesTypes(active_types)) {
            if (ShouldLog(hit)) {
                Logging.Log("[ai_bridge] %s v16_score blocked: unknown_types active=%u/%s switchin=%u/%s best=%d worst=%d\n",
        ...
        // Immediate lead utility: Fake Out is a tempo move.  If the AI already
        // scored Fake Out positively, do not pivot away from that active Pokémon.
        if (SummaryHasPositiveMove(252)) {
            if (ShouldLog(hit)) {
                Logging.Log("[ai_bridge] %s v16_score blocked: positive_fakeout active=%u/%s switchin=%u/%s best=%d worst=%d\n",
                    tag, static_cast<u32>(active_species), KnownSpeciesName(active_species), static_cast<u32>(switch_species), KnownSpeciesName(switch_species), best_move, worst_move);
            }
            return -9999;
        }
        ...
        for (u32 i = 0; i < move_record_count; i++) {
            ...
            if (IsKnownFriendlyCandidateSpecies(target_species, active_species, switch_species)) continue;
            threat_count++;
            ...
            const u32 target_stab_vs_active = MaxStabEffectiveness10(target_types, active_types);
            const u32 target_stab_vs_switch = MaxStabEffectiveness10(target_types, switch_types);
            const u32 switch_stab_vs_target = MaxStabEffectiveness10(switch_types, target_types);
        ...
        if (ShouldLog(hit)) {
            Logging.Log("[ai_bridge] %s v16_score active=%u/%s(%s/%s) switchin=%u/%s(%s/%s) score=%d best=%d worst=%d threats=%u known=%u bad=%u better=%u active_danger=%u offense=%u\n",
                tag,
    ...
    static inline bool V16MatchupGateAllows(u64 state_ptr, const char* tag, u32 hit) {
        const u16 switch_in_species = CandidateSwitchInSpecies(state_ptr);
        const u16 active_species = CandidateActiveSpecies(state_ptr);
        const s32 matchup_score = ScoreSwitchMatchupV16(active_species, switch_in_species, tag, hit);
        if (matchup_score < 30) {
            if (ShouldLog(hit)) {
                Logging.Log("[ai_bridge] %s v16_gate blocked score=%d threshold=30 active=%u/%s switchin=%u/%s\n",
        ...
        if (ShouldLog(hit)) {
            Logging.Log("[ai_bridge] %s v16_gate allowed score=%d active=%u/%s switchin=%u/%s\n",
                tag, matchup_score, static_cast<u32>(active_species), KnownSpeciesName(active_species), static_cast<u32>(switch_in_species), KnownSpeciesName(switch_in_species));
        }
        return true;
    }

    static inline bool SpeciesAwareSwitchGateAllows(u64 state_ptr, const char* tag, u32 hit) {
        if (global_config.ai_bridge.switch_policy_mode == 7) return V16MatchupGateAllows(state_ptr, tag, hit);
    ...
    static inline void ApplySwitchPolicyIfEnabled(u64 state_ptr, const char* tag, u32 hit) {
        ...
        const bool species_gate_allows_switch = gate_allows_switch ? SpeciesAwareSwitchGateAllows(state_ptr, tag, hit) : false;
        ...
        if (!gate_allows_switch || !species_gate_allows_switch) return;
        ...
        if (changed != 0 && ShouldLog(hit)) {
            Logging.Log("[ai_bridge] %s switch_policy_v16 mode=%u changed=%u total=%u a2=%u a3=%u other=%u cooldown=%u score=%u matching=%u preferred=%u native_only=%u\n",
                tag, mode, changed,
                AIBridge::policy_forced_total,
            ...
            DumpCandidateTable("candidate_score_after_switch_policy_v16", state_ptr, hit);
        }
    }
